...
test_wakeup(int all)
{
	...
	//test
	//printf("line: 527 from test_thread.c --> currentRunningThread is: %d \n", thread_id());

	/* create all threads */
	for (ii = 0; ii < NTHREADS; ii++) {
		//test
		//printf("line: 532 from test_thread.c --> currentRunningThread %d making children\n", thread_id());
...
test_wait_kill_thread(Tid parent)
{
	...
	if (ret != parent) { //parent has tid=0
...
test_wait_kill(void)
{
	...
	//test
	//printf("test_thread.c line 688 here \n ");
...
test_wait_exited(void)
{
	...
	//printf("I am thread: %d now creating child \n", thread_id());
	/* create a thread */
	...
	//printf("line 727 test_thread.c thread: %d here, test\n", thread_id());
